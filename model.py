...
class xxx4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(xxx4Rec, self).__init__(config, dataset)
...
class GLU(nn.Module):
    ...
    def forward(self, x):
        ...
        gated_value = value * self.act(gate)  #torch.sigmoid(gate)
...
class Layer(nn.Module):
    def __init__(self, d_model, d_state, d_conv, expand, n_heads, dropout, att_dropout, num_layers):
        ...
        #self.linatt = MultiHeadAttention(n_heads=n_heads, hidden_size=d_model, hidden_dropout_prob=dropout, attn_dropout_prob=att_dropout)
